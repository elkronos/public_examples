...
loadData <- function(dataInput) {
  if (is.character(dataInput)) {
    ...
    stop("dataInput must be a data frame or a string specifying the path to an Excel file.")
  }
  return(data)
}
...
treatment <- c("FLOOD", "FLOOD", "FLOOD", "FLOOD", "FLOOD", "FLOOD", "FLOOD", "FLOOD", "FLOOD", "AWD", "AWD", "AWD", "AWD", "AWD", "AWD", "AWD", "AWD", "AWD")
water <- c("NP", "NP", "NP", "HP", "HP", "HP", "OP", "OP", "OP", "NP", "NP", "NP", "HP", "HP", "HP", "OP", "OP", "OP")
priming <- c(4.60, 5.70, 4.10, 7.10, 5.60, 6.30, 8.00, 7.40, 7.80, 7.60, 8.70, 7.20, 10.10, 10.60, 9.30, 11.00, 10.40, 10.80)
aba <- c(0.90, 0.77, 0.81, 0.97, 0.91, 0.87, 1.73, 1.63, 1.45, 1.45, 1.30, 1.10, 1.95, 1.90, 1.85, 2.73, 2.63, 2.50)
apx <- c(3.78, 4.40, 5.33, 6.73, 6.31, 5.40, 8.38, 7.40, 7.63, 7.31, 6.40, 6.61, 9.31, 9.28, 8.64, 11.38, 11.43, 10.71)
pod <- c(91, 82, 87, 94, 87, 82, 110, 96, 92, 103, 89, 83, 104, 91, 84, 109, 100, 93)
ph <- c(11, 11, 10, 15, 14, 13, 17, 15, 14, 10, 13, 14, 17, 20, 18, 23, 19, 22)
til <- c(13.68, 16.90, 14.35, 18.43, 18.92, 15.36, 22.76, 20.42, 19.96, 20.89, 17.85, 16.27, 23.90, 20.85, 21.46, 27.29, 21.82, 23.45)
pl <- c(75.0, 84.5, 70.2, 94.1, 88.8, 81.1, 117.3, 104.6, 100.2, 95.3, 89.5, 81.2, 116.6, 100.4, 109.6, 150.2, 124.6, 137.3)
grp <- c(17.25, 13.63, 15.61, 20.89, 19.66, 17.37, 22.25, 22.65, 20.39, 18.60, 20.40, 17.09, 23.36, 21.40, 20.26, 29.33, 25.70, 26.40)
tgw <- c(20.92, 23.96, 19.12, 28.77, 25.90, 30.33, 36.38, 35.74, 30.78, 27.98, 31.50, 20.18, 33.01, 40.88, 35.11, 40.56, 45.68, 47.25)
gy <- c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)

# Create the data frame
data <- data.frame(rep, treatment, water, priming, aba, apx, pod, ph, til, pl, grp, tgw, gy)
